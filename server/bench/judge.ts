...
다음을 한국어 마크다운으로 매우 상세히, 근거를 인용하며 작성하라. 군더더기·서론 없이 바로:
- **종합**: 한 줄 평
- **강점**: 콘텐츠 정확도/자연스러움 등 (구체 근거)
- **약점**: 지시 준수(길이·포맷·제약), 할루시네이션, 사실/논리 오류 등 (구체 근거)
- **지시 준수**: 요청한 길이/형식/제약을 지켰는지 명시 판정
- **점수**: 정확도·지시준수·간결성 각 5점 만점`;
}
